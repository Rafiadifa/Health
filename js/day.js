...
const Day = (() => {
  ...
  const defaultCategoryFor = (k) => k === 'sweet' ? 'cake' : k === 'drink' ? 'unsweet' : 'rice';
  const kindPlaceholder = (k) => k==='manual'?'Food name':k==='sweet'?'e.g. Chocolate cake':k==='drink'?'e.g. Latte':'e.g. Beef & peppers';
  function newItem(kind='savory') {
    return { id:'it'+(itemSeq++), kind, name:'', category:defaultCategoryFor(kind), portion:1,
             oil:'medium', sauce:'some', cooking:'normal', richness:'normal', sweet:'sweet', calories:0 };
  }
  ...
  function createItemCard(item) {
    ...
    card.innerHTML = `
      <div class="item-card-head">
        <div class="kind-tabs">${['savory','sweet','drink','manual'].map(k=>`<button type="button" data-kind="${k}" class="${item.kind===k?'active':''}">${KIND_LABELS[k]}</button>`).join('')}</div>
    ...
    card.querySelectorAll('.kind-tabs button').forEach(btn => btn.addEventListener('click', () => {
      item.kind = btn.dataset.kind; item.category = defaultCategoryFor(item.kind);
      ...
      renderItemFields(card, item); recalcItem(item, card);
    }));
    ...
    const onChange = (e) => {
      ...
      item[f] = val; recalcItem(item, card);
    };
  ...
  function renderItemFields(card, item) {
    const w = card.querySelector('.item-fields');
    if (item.kind==='manual') {
      w.innerHTML = `<label class="ifield wide"><span>Calories (type directly)</span><input type="number" data-f="calories" placeholder="e.g. 450" value="${item.calories||''}" min="0" step="10" /></label>`;
    } else if (item.kind==='savory') {
      w.innerHTML = `<div class="field-grid">
        ...
        <label class="ifield"><span>Cooking</span><select data-f="cooking">${opts(COOKING_OPTS,item.cooking)}</select></label></div>`;
      ...
      w.innerHTML = `<div class="field-grid">
        ...
        <label class="ifield"><span>Portion</span><select data-f="portion">${opts(PORTIONS,item.portion)}</select></label>
        <label class="ifield"><span>Richness</span><select data-f="richness">${opts(RICHNESS_OPTS,item.richness)}</select></label></div>`;
      ...
      w.innerHTML = `<div class="field-grid">
        ...
        <label class="ifield"><span>Sweetness</span><select data-f="sweet">${opts(SWEET_OPTS,item.sweet)}</select></label></div>`;
    }
  }
  function recalcItem(item, card) { card.querySelector('.item-cal span').textContent = Calories.estimateItem(item)+' kcal'; recalcTotal(); }
  function recalcTotal() { $('mealTotal').textContent = Calories.mealTotal(currentItems)+' kcal'; }
  function addItem(kind) { const it=newItem(kind); currentItems.push(it); $('itemsContainer').appendChild(createItemCard(it)); recalcTotal(); }
  ...
  function openModal(entry) {
    ...
    if (entry) {
      items = (Array.isArray(entry.items) && entry.items.length)
        ? entry.items.map(it => ({ ...newItem(it.kind||'savory'), ...it, id:'it'+(itemSeq++) }))
        : [{ id:'it'+(itemSeq++), kind:'manual', name:entry.name||'Food', calories:entry.calories||0 }];
    } else items = [newItem('savory')];
  ...
  function saveFood() {
    ...
    const items = currentItems.map(it => ({ kind:it.kind, name:(it.name||'').trim(), category:it.category,
      portion:it.portion, oil:it.oil, sauce:it.sauce, cooking:it.cooking, richness:it.richness, sweet:it.sweet,
      calories: Calories.estimateItem(it) }));
    const entry = {
